...
func TestUpdate(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectExec("UPDATE users SET name = ?, surname = ?, email = ?, phone_number = ? WHERE id = ?").
		WithArgs(user.Name, user.Surname, user.Email, user.PhoneNumber, userId).
		WillReturnResult(sqlmock.NewResult(1, 1)) 

	mock.ExpectQuery("SELECT id, name, surname, email, phone_number, created_at FROM users WHERE id = ?").
		WithArgs(userId).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "surname", "email", "phone_number", "created_at"}).
			AddRow(userId, user.Name, user.Surname, user.Email, user.PhoneNumber, time.Now()))

	mock.ExpectCommit()

	updatedUser, err := repo.Update(context.Background(), userId, user)
	assert.NoError(t, err, "Expected no error while updating the user")
	assert.Equal(t, userId, updatedUser.Id, "Expected the updated user ID to match")
	assert.Equal(t, user.Name, updatedUser.Name, "Expected the updated user name to match")
	assert.Equal(t, user.Surname, updatedUser.Surname, "Expected the updated user surname to match")
	assert.Equal(t, user.Email, updatedUser.Email, "Expected the updated user email to match")
	assert.Equal(t, user.PhoneNumber, updatedUser.PhoneNumber, "Expected the updated user phone number to match")
